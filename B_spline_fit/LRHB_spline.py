...
def B_spline_boundary_calculate(knot_vector_block,vector,order):
...
def B_spline_point_derivative_calculate(control_point,knot_vector,vector,order,level,derivative_order=1):

    len_control_point = len(control_point[0])
...
def control_point_block_calculate_2d_level(data_point_block,first_tangent_vector,last_tangent_vector,knot_vector,knot_vector_block,order):

    max_knot_vector = max(knot_vector[0][0])

    len_data_point = len(data_point_block[0])

    control_point_block = np.zeros(shape = (2,len_data_point+2))

    data_point_matrix = data_point_block

    len_data_point_matrix  = len(data_point_matrix[0])
    ...
    for m in range(len_data_point_matrix):        
        ...
        for d in range(2):
            ...
            if (m == 0):
                ...
                LRHB_spline = LRHB_spline_boundary_calculate(knot_vector,knot_vector_block[order])

                data_point_matrix[d][0] = data_point_matrix[d][0]+first_tangent_vector[d]*LRHB_spline[order][0]*(knot_vector_block[order+1]-knot_vector_block[1])/LRHB_spline[order-1][1]/order/max_knot_vector

                coefficiency_matrix[m][0] = LRHB_spline[order][0]*(1-LRHB_spline[order-1][2]/LRHB_spline[order-1][1]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+2]-knot_vector_block[2]))+LRHB_spline[order][1]
                coefficiency_matrix[m][1] = LRHB_spline[order][0]*(LRHB_spline[order-1][2]/LRHB_spline[order-1][1]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+2]-knot_vector_block[2])-LRHB_spline[order-1][3]/LRHB_spline[order-1][1]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+3]-knot_vector_block[3]))+LRHB_spline[order][2]
                coefficiency_matrix[m][2] = LRHB_spline[order][0]*LRHB_spline[order-1][3]/LRHB_spline[order-1][1]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+3]-knot_vector_block[3])

            elif (m == len_data_point_matrix-1):

                LRHB_spline = LRHB_spline_boundary_calculate(knot_vector,knot_vector_block[m+order]-(knot_vector_block[m+order]-knot_vector_block[m+order-1])/1e6)

                data_point_matrix[d][m] = data_point_matrix[d][m]-last_tangent_vector[d]*LRHB_spline[order][m+2]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/LRHB_spline[order-1][m+1]/order/max_knot_vector

                coefficiency_matrix[m][m-2] = -LRHB_spline[order][m+2]*LRHB_spline[order-1][m-1]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order]-knot_vector_block[m])
                coefficiency_matrix[m][m-1] = LRHB_spline[order][m+2]*(LRHB_spline[order-1][m-1]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order]-knot_vector_block[m])-LRHB_spline[order-1][m]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order+1]-knot_vector_block[m+1]))+LRHB_spline[order][m]
                coefficiency_matrix[m][m] = LRHB_spline[order][m+2]*(LRHB_spline[order-1][m]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order+1]-knot_vector_block[m+1])-1)+LRHB_spline[order][m+1]
                ...
                for i in range(order):
                    ...
                    if (m+i-1 >= len_data_point_matrix):
                        ...
                        coefficiency_matrix[m][m+i-1] = LRHB_spline[order][m+i]

            else:

                LRHB_spline = LRHB_spline_calculate(knot_vector,knot_vector_block[m+order])
                ...
                for i in range(order):
                    ...
                    if (m+i-1 >= len_data_point_matrix):
                        ...
                        coefficiency_matrix[m][m+i-1] = LRHB_spline[m+i]
    ...
    for d in range(2):
        ...
        for m in range(1,len_data_point+1):

            control_point_block[d][m] = control_point_matrix[d][m-1]

        LRHB_spline = LRHB_spline_boundary_calculate(knot_vector,knot_vector_block[order])

        if(LRHB_spline[order-1][0] == 0):

            control_point_block[d][0] = control_point_block[d][1]*(1-LRHB_spline[order-1][1]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+2]-knot_vector_block[2]))+control_point_block[d][2]*(LRHB_spline[order-1][1]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+2]-knot_vector_block[2])-LRHB_spline[order-1][2]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+3]-knot_vector_block[3]))+control_point_block[d][3]*LRHB_spline[order-1][2]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+3]-knot_vector_block[3])-first_tangent_vector[d]*(knot_vector_block[order+1]-knot_vector_block[1])/order/max_knot_vector

        else:

            control_point_block[d][0] = control_point_block[d][1]*(1-LRHB_spline[order-1][1]/LRHB_spline[order-1][0]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+2]-knot_vector_block[2]))+control_point_block[d][2]*(LRHB_spline[order-1][1]/LRHB_spline[order-1][0]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+2]-knot_vector_block[2])-LRHB_spline[order-1][2]/LRHB_spline[order-1][0]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+3]-knot_vector_block[3]))+control_point_block[d][3]*LRHB_spline[order-1][2]/LRHB_spline[order-1][0]*(knot_vector_block[order+1]-knot_vector_block[1])/(knot_vector_block[order+3]-knot_vector_block[3])-first_tangent_vector[d]*(knot_vector_block[order+1]-knot_vector_block[1])/LRHB_spline[order-1][0]/order/max_knot_vector
        
        m = len_data_point_matrix-1

        LRHB_spline = LRHB_spline_boundary_calculate(knot_vector,knot_vector_block[m+order]-(knot_vector_block[m+order]-knot_vector_block[m+order-1])/1e6)

        control_point_block[d][len_data_point+1] = last_tangent_vector[d]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/LRHB_spline[order-1][m+1]/order/max_knot_vector-control_point_block[d][len_data_point-2]*LRHB_spline[order-1][m-1]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order]-knot_vector_block[m])+control_point_block[d][len_data_point-1]*(LRHB_spline[order-1][m-1]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order]-knot_vector_block[m])-LRHB_spline[order-1][m]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order+1]-knot_vector_block[m+1]))+control_point_block[d][len_data_point]*(LRHB_spline[order-1][m]/LRHB_spline[order-1][m+1]*(knot_vector_block[m+order+2]-knot_vector_block[m+2])/(knot_vector_block[m+order+1]-knot_vector_block[m+1])-1)
...
def control_point_block_calculate_3d_level(control_point,control_point_value,data_point,data_point_value,knot_vector,order,block,block_0,level):
    
    data_point_1_block = data_point_block_add(data_point[0],block,level)
    data_point_2_block = data_point_block_add(data_point[1],block,level)
    knot_vector_1_sequence = knot_vector_sequence_find(knot_vector[0],0,block,level)
    knot_vector_2_sequence = knot_vector_sequence_find(knot_vector[1],0,block,level)

    len_data_point_1 = len(data_point_1_block)
    len_data_point_2 = len(data_point_2_block)

    il1 = position_find(knot_vector_1_sequence[level-1],knot_vector_1_sequence[level][order[0]])
    il2 = position_find(knot_vector_2_sequence[level-1],knot_vector_2_sequence[level][order[1]])

    il1 = il1-order[0]
    il2 = il2-order[1]

    control_point_1_block = np.zeros(shape = (3,len_data_point_1+2,len_data_point_2))
    ...
    for n2 in range(len_data_point_2):
        ...
        for n1 in range(len_data_point_1):
            ...
            dp[1][n1] = data_point_value[level][block][n1][n2]

        cp1 = np.zeros(shape = (2,len(control_point[0][level-1][block_0])))

        for m1 in range(len(control_point[0][level-1][block_0])):

            cp1[0][m1] = control_point[0][level-1][block_0][m1]
            cp1[1][m1] = control_point_value[level-1][block_0][m1][il2+1]

        data_point_derivative_level = data_point_derivative_level_calculate_2d(cp1,knot_vector_1_sequence[level-1],[knot_vector[0][level-1][block_0][il1],knot_vector[0][level-1][block_0][il1+1]],level)

        first_tangent_vector = data_point_derivative_level[0]
        last_tangent_vector = data_point_derivative_level[1]

        cp = control_point_block_calculate_2d_level(dp,first_tangent_vector,last_tangent_vector,knot_vector[0],knot_vector_1_sequence[level],order[0])

        for n1 in range(len_data_point_1+2):

            control_point_1_block[0][n1][n2] = cp[0][n1]
            control_point_1_block[1][n1][n2] = data_point_2_block[n2]
            control_point_1_block[2][n1][n2] = cp[1][n1]
    ...
    for n1 in range(len_data_point_1+2):
        ...
        for n2 in range(len_data_point_2):
            ...
            dp[0][n2] = control_point_1_block[1][n1][n2]
            dp[1][n2] = control_point_1_block[2][n1][n2]

        cp2 = np.zeros(shape = (2,len(control_point[1][level-1][block_0])))

        for m2 in range(len(control_point[1][level-1][block_0])):

            cp2[0][m2] = control_point[1][level-1][block_0][m2]
            cp2[1][m2] = control_point_value[level-1][block_0][il1+1][m2]

        data_point_derivative_level = data_point_derivative_level_calculate_2d(cp2,knot_vector_2_sequence[level-1],[knot_vector[1][level-1][block_0][il2],knot_vector[1][level-1][block_0][il2+1]],level)

        first_tangent_vector = data_point_derivative_level[0]
        last_tangent_vector = data_point_derivative_level[1]

        cp = control_point_block_calculate_2d_level(dp,first_tangent_vector,last_tangent_vector,knot_vector[1],knot_vector_2_sequence[level],order[1])

        for n2 in range(len_data_point_2+2):

            control_point_block[0][n1][n2] = control_point_1_block[0][n1][0]
...
def data_point_block_add(data_point,block,level):
    ...
    data_point_position = data_point_position_find_2d(data_point,0,block,level)

    data_point_block.insert(0,data_point[data_point_position[2]][data_point_position[1]][data_point_position[0]])

    data_point_position = data_point_position_find_2d(data_point,len(data_point[level][block])-1,block,level)
...
def data_point_block_find(data_point_level,data_point_sequence):

    b = data_point_level.index(data_point_sequence)
...
def data_point_derivative_level_calculate_2d(control_point_level,knot_vector_level_0,vector,level,derivative_order=1):
    ...
    first_tangent_vector = B_spline_point_derivative_calculate(control_point_level,knot_vector_level_0,vector[0],order,level,derivative_order)
    last_tangent_vector = B_spline_point_derivative_calculate(control_point_level,knot_vector_level_0,vector[1],order,level,derivative_order)
...
def data_point_position_find_2d(data_point,number,block,level):
...
def data_point_sequence_find(data_point,number,block,level):
    ...
    for l in range(level):
        ...
        data_point_position = data_point_position_find_2d(data_point,data_point_number,data_point_block,data_point_level)
        ...
        if (data_point_position[2] == data_point_level-1 or data_point_position[5] == data_point_level-1):
            ...
            if (data_point_position[2] != 0):
                ...
                while True:
                    ...
                    data_point_position = data_point_position_find_2d(data_point,data_point_position[0],data_point_position[1],data_point_position[2])
            ...
            if (data_point_position[5] != 0):
                ...
                while True:
                    ...
                    data_point_position = data_point_position_find_2d(data_point,data_point_position[3],data_point_position[4],data_point_position[5])
...
def data_point_value_position_find(data_point_1,data_point_2,new_point):

    position_1 = list(vector_position_find_2d(data_point_1,new_point[0]))
    position_2 = list(vector_position_find_2d(data_point_2,new_point[1]))

    max_level = max(position_1[2],position_1[5],position_2[2],position_2[5])

    if (position_1[2] == position_2[2] and position_1[2] == max_level and position_2[2] == max_level and position_1[1] == position_2[1]):

        loop_status = False

    elif (position_1[5] == position_2[5] and position_1[5] == max_level and position_2[5] == max_level and position_1[4] == position_2[4]):

        loop_status = False

    elif (position_1[2] == position_2[5] and position_1[2] == max_level and position_2[5] == max_level and position_1[1] == position_2[4]):

        loop_status = False

    elif (position_1[5] == position_2[2] and position_1[5] == max_level and position_2[2] == max_level and position_1[4] == position_2[1]):
    ...
    while (loop_status):
        ...
        if (position_1[2] == max_level):

            data_point_position_1 = data_point_position_find_2d(data_point_1,position_1[0],position_1[1],position_1[2])

            position_1[0] = data_point_position_1[0]
            position_1[1] = data_point_position_1[1]
            position_1[2] = data_point_position_1[2]

        if (position_1[5] == max_level):

            data_point_position_1 = data_point_position_find_2d(data_point_1,position_1[3],position_1[4],position_1[5])

            position_1[3] = data_point_position_1[3]
            position_1[4] = data_point_position_1[4]
            position_1[5] = data_point_position_1[5]

        if (position_2[2] == max_level):

            data_point_position_2 = data_point_position_find_2d(data_point_2,position_2[0],position_2[1],position_2[2])

            position_2[0] = data_point_position_2[0]
            position_2[1] = data_point_position_2[1]
            position_2[2] = data_point_position_2[2]

        if (position_2[5] == max_level):

            data_point_position_2 = data_point_position_find_2d(data_point_2,position_2[3],position_2[4],position_2[5])

            position_2[3] = data_point_position_2[3]
            position_2[4] = data_point_position_2[4]
            position_2[5] = data_point_position_2[5]
    ...
    if (position_1[2] == position_2[2] and position_1[2] == max_level and position_2[2] == max_level and position_1[1] == position_2[1]):
        ...
        if (level == 0):
            ...
            left_number_1 = position_1[0]
            right_number_1 = position_1[0]+1
            left_number_2 = position_2[0]
            right_number_2 = position_2[0]+1

        else:

            left_number_1 = position_1[0]+1
            right_number_1 = position_1[0]+2
            left_number_2 = position_2[0]+1
            right_number_2 = position_2[0]+2
        ...
        if (level == 0):
            ...
            left_number_1 = position_1[3]-1
            right_number_1 = position_1[3]
            left_number_2 = position_2[3]-1
            right_number_2 = position_2[3]

        else:

            left_number_1 = position_1[3]
            right_number_1 = position_1[3]+1
            left_number_2 = position_2[3]
            right_number_2 = position_2[3]+1
    ...
    return level,block,left_number_1,right_number_1,left_number_2,right_number_2

def data_point_value_sequence_find(data_point,data_point_value,number,block,level):

    data_point_value_sequence = []
    ...
    for l in range(level):
        ...
        data_point_value_sequence_level = data_point_value[data_point_level][data_point_block]
        ...
        data_point_value_sequence.insert(0,data_point_value_sequence_level)

    data_point_value_sequence_level = data_point_value[0][0]

    data_point_value_sequence.insert(0,data_point_value_sequence_level)

    return data_point_value_sequence

def fit_2d(data_point,data_point_value,point_number):

    level = len(data_point)
    ...
    for l in range(level):
        ...
        if (l == 0):
            ...
            control_point_calculate = LR_control_point_calculate_2d(data_point,data_point_value,knot_vector)

            control_point = control_point_calculate[0]
            control_point_value = control_point_calculate[1]

        else:

            knot_vector = knot_vector_calculate_2d_level(data_point,data_point_value,knot_vector,l)

            control_point_calculate = LR_control_point_calculate_2d_level(control_point,control_point_value,data_point,data_point_value,knot_vector,l)

            control_point = control_point_calculate[0]
            control_point_value = control_point_calculate[1]

    vector_delta = max(knot_vector[0][0])/point_number
    point_vector = np.zeros(point_number+1)

    for u in range(point_number):

        point_vector[u] = u*vector_delta

    fit_point = np.zeros(shape = (2,point_number))
    ...
    for n in range(point_number):
        ...
        fit_point[0][n] = LRHB_spline_point[0]
        fit_point[1][n] = LRHB_spline_point[1]

    return fit_point

def fit_2d_3d(control_point,control_point_value,knot_vector,point_number):
...
def fit_3d(data_point,data_point_value,point_number):

    level = len(data_point[0])

    for l in range(level):

        if (l == 0):    

            knot_vector_calculate = knot_vector_calculate_3d(data_point,data_point_value)

            control_point_calculate = LR_control_point_calculate_3d(data_point,data_point_value,knot_vector_calculate)

        else:

            knot_vector_calculate = knot_vector_calculate_3d_level(data_point,data_point_value,knot_vector_calculate,l)

            control_point_calculate = LR_control_point_calculate_3d_level([control_point_calculate[0],control_point_calculate[1]],control_point_calculate[2],data_point,data_point_value,knot_vector_calculate,l)
...
def LR_control_point_calculate_2d(data_point,data_point_value,knot_vector):
    ...
    control_point_block_calculate = control_point_block_calculate_2d([data_point_block,data_point_value_block],first_tangent_vector,last_tangent_vector,knot_vector_block,order)

    control_point = [[list(control_point_block_calculate[0])]]
    control_point_value = [[list(control_point_block_calculate[1])]]

    return control_point,control_point_value

def LR_control_point_calculate_2d_level(control_point,control_point_value,data_point,data_point_value,knot_vector,level):

    len_data_point_block = len(data_point[level])
    ...
    for b in range(len_data_point_block):
        ...
        data_point_sequence = data_point_sequence_find(data_point,0,b,level)
        b1 = data_point_block_find(data_point[level-1],data_point_sequence[level-1])
    
        data_point_block = data_point_block_add(data_point,b,level)
        data_point_value_block = list(data_point_value[level][b])

        knot_vector_sequence = knot_vector_sequence_find(knot_vector,0,b,level)
        ...
        il1 = position_find(knot_vector_sequence[level-1],knot_vector_sequence[level][order])

        # il1 = il1-order

        data_point_derivative_level = data_point_derivative_level_calculate_2d([control_point[level-1][b1],control_point_value[level-1][b1]],knot_vector_sequence[level-1],[knot_vector[level-1][b1][il1],knot_vector[level-1][b1][il1+1]],level)

        first_tangent_vector = data_point_derivative_level[0]
        last_tangent_vector = data_point_derivative_level[1]

        control_point_block_calculate = control_point_block_calculate_2d_level([data_point_block,data_point_value_block],first_tangent_vector,last_tangent_vector,knot_vector,knot_vector_sequence[level],order)

        control_point_level.append(list(control_point_block_calculate[0]))
        control_point_value_level.append(list(control_point_block_calculate[1]))
...
def LR_control_point_calculate_3d(data_point,data_point_value,knot_vector):

    data_point_1 = data_point[0]
    data_point_2 = data_point[1]
    knot_vector_1 = knot_vector[0]
    knot_vector_2 = knot_vector[1]
...
def LR_control_point_calculate_3d_level(control_point,control_point_value,data_point,data_point_value,knot_vector,level):
    ...
    len_data_point_block = len(data_point_1[level])
    ...
    for b in range(len_data_point_block):
        ...
        data_point_1_sequence = data_point_sequence_find(data_point_1,0,b,level)
        b1 = data_point_block_find(data_point_1[level-1],data_point_1_sequence[level-1])

        data_point_1_block = data_point_block_add(data_point_1,b,level)
        data_point_2_block = data_point_block_add(data_point_2,b,level)

        order = []

        if (len(data_point_1_block) < 4):
        ...
        control_point_block = control_point_block_calculate_3d_level(control_point,control_point_value,data_point,data_point_value,knot_vector,order,b,b1,level)
...
def LRHBS_control_point_block_calculate_3d(control_point_1,control_point_2,control_point_value,data_point_1,data_point_2,data_point_value,knot_vector_1,knot_vector_2,block,level):
    ...
    data_point_1_sequence = data_point_sequence_find(data_point_1,0,b0,level-1)
    data_point_2_sequence = data_point_sequence_find(data_point_2,0,b0,level-1)
    ...
    control_point_value_sequence = data_point_sequence_find(control_point_value,0,b0,level-1)
    ...
    for n2 in range(len_data_point_2):
        ...
        cp = control_point_block_calculate_2d_level([control_point_1_sequence,cpvs],dp,first_tangent_vector,last_tangent_vector,knot_vector_1,knot_vector_1_sequence,level)
    ...
    for n1 in range(len_data_point_1+2):
        ...
        cp = control_point_block_calculate_2d_level([control_point_2_sequence,cpvs],dp,first_tangent_vector,last_tangent_vector,knot_vector_2,knot_vector_2_sequence,level)
...
def LRHB_spline_calculate(knot_vector,vector):
    ...
    vector_position = vector_position_find_2d(knot_vector,vector)
    ...
    if (level == 0):
        ...
        knot_vector_sequence_level = knot_vector_sequence[level]

        if (len(knot_vector[level][block]) < 8):
...
def LRHB_spline_boundary_calculate(knot_vector,vector):
    ...
    if (level == 0):
        ...
        LRHB_spline = B_spline_boundary_calculate(knot_vector_level,vector,order)

    else:

        knot_vector_sequence_level = knot_vector_sequence[level]

        if (len(knot_vector_sequence[level]) < 8):

            order = 2

            LRHB_spline = B_spline_boundary_calculate(knot_vector_sequence_level,vector,order)
...
def LRHB_spline_point_calculate_2d(control_point,knot_vector,vector):
    ...
    len_control_point = len(control_point[0][level][block])

    for d in range(2):

        for m in range(4):

            if (number+m >= len_control_point):

                continue

            else:

                point[d] = point[d]+control_point[d][level][block][number+m]*LRHB_spline[number+m]

    return point

def LRHB_spline_point_calculate_3d(control_point,knot_vector,vector):
    ...
    vector_position = vector_position_find_3d(knot_vector_1,knot_vector_2,vector)

    level = vector_position[0]
    block = vector_position[1]
    number_1 = vector_position[2]
    number_2 = vector_position[4]
    ...
    i1 = position_find(knot_vector_1_sequence[level],vector[0])
    i2 = position_find(knot_vector_2_sequence[level],vector[1])
    ...
    if (len(knot_vector_1_sequence[level]) < 4):
    ...
    if (len(knot_vector_2_sequence[level]) < 4):
    ...
    for m1 in range(order[0]+1):
        ...
        for m2 in range(order[1]+1):
            ...
            if (i2+m2 >= len(control_point_2[level][block])):

                continue

            else:

                point_1_block[0] = point_1_block[0]+control_point_2[0][0][i2+m2]*LRHB_spline_2[i2+m2]
                point_1_block[1] = point_1_block[1]+control_point_value[0][0][i1+m1][i2+m2]*LRHB_spline_2[i2+m2]

        if (i1+m1 >= len(control_point_1[level][block])):

            continue

        else:

            point[0] = point[0]+control_point_1[0][0][i1+m1]*LRHB_spline_1[i1+m1]
            point[1] = point[1]+point_1_block[0]*LRHB_spline_1[i1+m1]
            point[2] = point[2]+point_1_block[1]*LRHB_spline_1[i1+m1]
...
def knot_vector_calculate_2d_level(data_point,data_point_value,knot_vector,level):
    ...
    for b in range(len_data_point_block):
        ...
        len_data_point = len(data_point[level][b])
        
        knot_vector_block = list(np.zeros(len_data_point))

        data_point_position = data_point_position_find_2d(data_point,0,b,level)

        if (data_point_position[2] != level-2 and data_point_position[5] != level-2):

            for n in range(len_data_point):

                data_point_sequence = data_point_sequence_find(data_point,n,b,level)

                il1 = position_find(data_point_sequence[level-1],data_point_sequence[level][n])
                il = position_find(data_point_sequence[level],data_point_sequence[level][n])

                b1 = data_point_block_find(data_point[level-1],list(np.squeeze(data_point_sequence[level-1])))

                distance_sum = 0

                for nd in range(len(data_point_sequence[level])+1):

                    if (nd == 0):

                        if (level-1 == 0):

                            distance_sum = distance_sum+data_point_distance_calculate([data_point[level-1][b1][il1],data_point[level][b][nd]],[data_point_value[level-1][b1][il1],data_point_value[level][b][nd+1]])

                        else:

                            distance_sum = distance_sum+data_point_distance_calculate([data_point[level-1][b1][il1],data_point[level][b][nd]],[data_point_value[level-1][b1][il1+1],data_point_value[level][b][nd+1]])

                        if (nd == il):

                            knot_distance = distance_sum
                        
                    elif (nd == len(data_point_sequence[level])):

                        if (level-1 == 0):

                            distance_sum = distance_sum+data_point_distance_calculate([data_point[level][b][nd-1],data_point[level-1][b1][il1+1]],[data_point_value[level][b][nd],data_point_value[level-1][b1][il1+1]])

                        else:

                            distance_sum = distance_sum+data_point_distance_calculate([data_point[level][b][nd-1],data_point[level-1][b1][il1+1]],[data_point_value[level][b][nd],data_point_value[level-1][b1][il1+2]])

                    else:

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[level][b][nd-1],data_point[level][b][nd]],[data_point_value[level][b][nd],data_point_value[level][b][nd+1]])

                        if (nd == il):

                            knot_distance = distance_sum

                knot_vector_block[n] = knot_vector[level-1][b1][il1]+(knot_vector[level-1][b1][il1+1]-knot_vector[level-1][b1][il1])*knot_distance/distance_sum
            ...
            for n in range(len_data_point):
                ...
                data_point_position = data_point_position_find_2d(data_point,n,b,level)

                ll = 0
                rl = 0

                if (data_point_position[2] != level and data_point_position[5] != level):
                ...
                elif (data_point_position[2] != level and data_point_position[5] == level):
                    ...
                    while (rl == 0):
                        ...
                        if (data_point_position[5] != level):
                    ...
                    while (rl == 0):
                        ...
                        if (data_point_position[2] != level):
                ...
                for nd in range(len(data_point_sequence[level])+1):
                    ...
                    if (nd == 0):
                        ...
                        if (ll == 0):
                            ...
                            distance_sum = distance_sum+data_point_distance_calculate([data_point[ll][lb][ln],data_point[level][b][nd]],[data_point_value[ll][lb][ln],data_point_value[level][b][nd+1]])

                        else:

                            distance_sum = distance_sum+data_point_distance_calculate([data_point[ll][lb][ln],data_point[level][b][nd]],[data_point_value[ll][lb][ln+1],data_point_value[level][b][nd+1]])
                        ...
                        if (rl == 0):
                            ...
                            distance_sum = distance_sum+data_point_distance_calculate([data_point[level][b][nd-1],data_point[rl][rb][rn]],[data_point_value[level][b][nd],data_point_value[rl][rb][rn]])

                        else:

                            distance_sum = distance_sum+data_point_distance_calculate([data_point[level][b][nd-1],data_point[rl][rb][rn]],[data_point_value[level][b][nd],data_point_value[rl][rb][rn+1]])
...
def knot_vector_calculate_3d_level(data_point,data_point_value,knot_vector,level):
    ...
    for l in range(level):
        ...
        data_point_1.append(data_point[0][l])
        data_point_2.append(data_point[1][l])

    knot_vector_1 = knot_vector[0]
    knot_vector_2 = knot_vector[1]

    len_data_point_block = len(data_point[0][level])
    ...
    for b in range(len_data_point_block):
        ...
        len_data_point_1 = len(data_point[0][level][b])
        len_data_point_2 = len(data_point[1][level][b])
        
        knot_vector_1_block_total = []

        data_point_position = vector_position_find_3d(data_point_1,data_point_2,[data_point[0][level][b][0],data_point[1][level][b][0]])

        ll = data_point_position[0]
        rl = ll
        lb = data_point_position[1]
        rb = lb
        ln = data_point_position[2]
        rn = ln+1
        ...
        for n2 in range(len_data_point_2):
            ...
            for n1 in range(len_data_point_1):
                ...
                i0 = position_find(data_point[0][level][b],data_point[0][level][b][n1])

                distance_sum = 0

                for nd in range(len(data_point[0][level][b])+1):

                    if (nd == 0):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[0][ll][lb][ln],data_point[0][level][b][nd]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])

                        if (nd == i0):

                            knot_distance = distance_sum
                        
                    elif (nd == len(data_point[0][level][b])):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[0][level][b][nd-1],data_point[0][rl][rb][rn]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])

                    else:

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[0][level][b][nd-1],data_point[0][level][b][nd]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])
        ...
        for n1 in range(len_data_point_1):
            ...
            for n2 in range(len_data_point_2):
                ...
                i0 = position_find(data_point[1][level][b],data_point[1][level][b][n2])

                distance_sum = 0

                for nd in range(len(data_point[1][level][b])+1):

                    if (nd == 0):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[1][ll][lb][ln],data_point[1][level][b][nd]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])

                        if (nd == i0):

                            knot_distance = distance_sum
                        
                    elif (nd == len(data_point[1][level][b])):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[1][level][b][nd-1],data_point[1][rl][rb][rn]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])

                    else:

                        distance_sum = distance_sum+data_point_distance_calculate([data_point[1][level][b][nd-1],data_point[1][level][b][nd]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])
...
def knot_vector_calculate_3d_level_1(data_point,data_point_value,knot_vector,level):
    ...
    for b in range(len_data_point_block):
        ...
        len_data_point_1 = len(data_point_1[level][b])
        len_data_point_2 = len(data_point_2[level][b])
        
        knot_vector_1_block_total = []

        data_point_1_position = data_point_position_find_2d(data_point_1,0,b,level)
        ...
        for n2 in range(len_data_point_2):
            ...
            for n1 in range(len_data_point_1):
                ...
                data_point_1_sequence = data_point_sequence_find(data_point_1,n1,b,level)

                i0 = position_find(data_point_1_sequence[level],data_point_1_sequence[level][n1])

                data_point_1_position = data_point_position_find_2d(data_point_1,n1,b,level)

                left_condition = True
                right_condition = True

                if (data_point_1_position[2] != level and data_point_1_position[5] != level):
                ...
                elif (data_point_1_position[2] != level and data_point_1_position[5] == level):
                    ...
                    while (right_condition):
                        ...
                        data_point_1_position = data_point_position_find_2d(data_point_1,data_point_1_position[3],data_point_1_position[4],data_point_1_position[5])

                        if (data_point_1_position[5] != level):
                    ...
                    while (left_condition):
                        ...
                        data_point_1_position = data_point_position_find_2d(data_point_1,data_point_1_position[0],data_point_1_position[1],data_point_1_position[2])

                        if (data_point_1_position[2] != level):
                ...
                for nd in range(len(data_point_1_sequence[level])+1):

                    if (nd == 0):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point_1[ll][lb][ln],data_point_1[level][b][nd]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])

                        if (nd == i0):

                            knot_distance = distance_sum
                        
                    elif (nd == len(data_point_1_sequence[level])):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point_1[level][b][nd-1],data_point_1[rl][rb][rn]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])

                    else:

                        distance_sum = distance_sum+data_point_distance_calculate([data_point_1[level][b][nd-1],data_point_1[level][b][nd]],[data_point_value[level][b][nd][n2+1],data_point_value[level][b][nd+1][n2+1]])
    ...
    len_data_point_block = len(data_point_2[level])
    ...
    for b in range(len_data_point_block):
        ...
        data_point_2_position = data_point_position_find_2d(data_point_2,0,b,level)
        ...
        for n1 in range(len_data_point_1):
            ...
            for n2 in range(len_data_point_2):
                ...
                data_point_2_sequence = data_point_sequence_find(data_point_2,n2,b,level)

                i0 = position_find(data_point_2_sequence[level],data_point_2_sequence[level][n2])

                data_point_2_position = data_point_position_find_2d(data_point_2,n2,b,level)

                left_condition = True
                right_condition = True

                if (data_point_2_position[2] != level and data_point_2_position[5] != level):
                ...
                elif (data_point_2_position[2] != level and data_point_2_position[5] == level):
                    ...
                    while (right_condition):
                        ...
                        data_point_2_position = data_point_position_find_2d(data_point_2,data_point_2_position[3],data_point_2_position[4],data_point_2_position[5])

                        if (data_point_2_position[5] != level):
                    ...
                    while (left_condition):
                        ...
                        data_point_2_position = data_point_position_find_2d(data_point_2,data_point_2_position[0],data_point_2_position[1],data_point_2_position[2])

                        if (data_point_2_position[2] != level):
                ...
                for nd in range(len(data_point_2_sequence[level])+1):

                    if (nd == 0):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point_2[ll][lb][ln],data_point_2[level][b][nd]],[data_point_value[level][b][n1+1][nd],data_point_value[level][b][n1+1][nd+1]])

                        if (nd == i0):

                            knot_distance = distance_sum
                        
                    elif (nd == len(data_point_2_sequence[level])):

                        distance_sum = distance_sum+data_point_distance_calculate([data_point_2[level][b][nd-1],data_point_2[rl][rb][rn]],[data_point_value[level][b][n1+1][nd],data_point_value[level][b][n1+1][nd+1]])

                    else:

                        distance_sum = distance_sum+data_point_distance_calculate([data_point_2[level][b][nd-1],data_point_2[level][b][nd]],[data_point_value[level][b][n1+1][nd],data_point_value[level][b][n1+1][nd+1]])
...
def knot_vector_sequence_find(knot_vector,number,block,level):
    ...
    for level in range(level+1):
        ...
        knot_vector_sequence_level = list(data_sequence[level])

        if (level > 0):

            if (len(data_sequence[level]) < 2):
            ...
            i1 = position_find(knot_vector_sequence[level-1],data_sequence[level][0])

            len_kvs = len(knot_vector_sequence[level-1])

            for m in range(order+1):

                if (i1-m < 0 and i1+1+m >= len_kvs):

                    knot_vector_sequence_level.insert(0,knot_vector_sequence[level-1][0])
                    knot_vector_sequence_level.append(knot_vector_sequence[level-1][len_kvs-1])

                elif (i1-m < 0):

                    knot_vector_sequence_level.insert(0,knot_vector_sequence[level-1][0])
                    knot_vector_sequence_level.append(knot_vector_sequence[level-1][i1+1+m])

                elif (i1+1+m >= len_kvs):

                    knot_vector_sequence_level.insert(0,knot_vector_sequence[level-1][i1-m])
                    knot_vector_sequence_level.append(knot_vector_sequence[level-1][len_kvs-1])
...
def vector_position_find_2d(knot_vector,vector):
    ...
    for level in range(knot_vector_level):

        len_knot_vector_block = len(knot_vector[level])

        for b in range(len_knot_vector_block):

            len_knot_vector = len(knot_vector[level][b])

            for n in range(len_knot_vector):

                dif = vector-knot_vector[level][b][n]
...
def vector_position_find_3d(knot_vector_1,knot_vector_2,vector):

    knot_vector_level = len(knot_vector_1)

    for l in range(knot_vector_level):

        len_knot_vector_block = len(knot_vector_1[l])

        for b in range(len_knot_vector_block):

            len_knot_vector_1 = len(knot_vector_1[l][b])
            len_knot_vector_2 = len(knot_vector_2[l][b])

            for n1 in range(len_knot_vector_1-1):

                for n2 in range(len_knot_vector_2-1):

                    if (vector[0] >= knot_vector_1[l][b][n1] and vector[0] <= knot_vector_1[l][b][n1+1] and vector[1] >= knot_vector_2[l][b][n2] and vector[1] <= knot_vector_2[l][b][n2+1]):

                        level = l
                        block = b

    len_knot_vector_1 = len(knot_vector_1[level][block])

    left_number_1 = len_knot_vector_1-1
    right_number_1 = len_knot_vector_1

    for n1 in range(len_knot_vector_1-1):

        if (vector[0] >= knot_vector_1[level][block][n1] and vector[0] < knot_vector_1[level][block][n1+1]):

            left_number_1 = n1
            right_number_1 = n1+1
            break

    len_knot_vector_2 = len(knot_vector_2[level][block])

    left_number_2 = len_knot_vector_2-1
    right_number_2 = len_knot_vector_2

    for n2 in range(len_knot_vector_2-1):

        if (vector[1] >= knot_vector_2[level][block][n2] and vector[1] < knot_vector_2[level][block][n2+1]):
...
def vector_sequence_find(knot_vector,vector):
    ...
    for level in range(level):
        ...
        vector_position = data_point_position_find_2d(knot_vector,knot_vector_number,knot_vector_level)
        ...
        if (vector_position[2] == knot_vector_level or vector_position[5] == knot_vector_level):
            ...
            if (vector_position[2] == knot_vector_level):
                ...
                for n in range(len_knot_vector_level):
                    ...
                    vector_position = data_point_position_find_2d(knot_vector,vector_position[0],vector_position[1])
            ...
            if(vector_position[3] == knot_vector_level):
                ...
                for n in range(len_knot_vector_level):
                    ...
                    vector_position = data_point_position_find_2d(knot_vector,vector_position[2],vector_position[3])
